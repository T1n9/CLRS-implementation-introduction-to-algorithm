...
public:
    rb_tree();
    ~rb_tree();
    void interval_search(const Pair &);
    bool insert(const int&low, const int &high);
    ...
    void interval_tree_fix(node* pNode);
    void create_node(node** pNode, node* parent, _color color, const int& low, const int& high);
    ...
    void case_5(node*);
    void case_6(node*);
    void maintain_size(node*);
    int os_rank(const int&);
    int os_select(const int& rank, node *NODE = root);
    int os_select_non_recursive(const int& rank);
...
void rb_tree::interval_search(const Pair & value)
{
    node* walk = root;
    while(walk!=NIL)
    {
        if(value.low <= walk->high && walk->data < value.high)
        {
            cout<<"found overlap for value with high "<<value.high<<" and low "<<value.low<<endl;
            return;
        }
        if(walk->left!=NIL && walk->left->max>= value.low)
            walk = walk->left;
        else walk = walk->right;
    }
    cout<<"overlap for data with high "<<value.high<<" and low "<<value.low <<" does not exist in red-black tree"<<endl;
    return;
}
int rb_tree::os_select(const int & rank, node* NODE)
{
    if(rank > root->size)
    {
        cout<<"the rank you want is bigger than the tree size"<<endl;
        return INT_MIN;
    }
    ...
    if(rank == r)
       ...
       return os_select(rank, NODE->left);
    else return os_select(rank-r, NODE->right);
}
int rb_tree::os_select_non_recursive(const int &rank)
{
    int rankTmp = rank;
    if(rankTmp > root->size)
    {
        cout<<"error: rank you want bigger than tree size"<<endl;
        return INT_MIN;
    }

    node* walk = root;
    int r = walk->left->size +1;
    while(r != rankTmp)
    {
        if(rankTmp < r)
        {
...
int rb_tree::os_rank(const int & value)
{
    node* res = search_data(value);
    if(res == NIL)
    {
        cout<<"cannot find the value in tree";
        return INT_MIN;
    }
    else
    {
        int r = res->left->size+1;
        node* tmp = res;
        while(tmp!=root)
        {
            if(tmp == tmp->parent->right)
                r = r+ tmp->parent->left->size +1;
            tmp = tmp->parent;
        }
        return r;
    }
}
void rb_tree::maintain_size(node * walk)
{
    walk = walk->parent;
    while(walk!=NULL)
    {
...
//    cout<<"x:"<<x_node->data<<endl;
    ...
    y_node->size = y_node->right->size + y_node->left->size +1;
}
void rb_tree::show(node* NODE)
{
    if(NODE!=NIL)
    {
    cout<<"low:"<<NODE->data<<". high:"<<NODE->high<<". max:"<<NODE->max<<endl;
    show(NODE->left);
    show(NODE->right);
    }
}
void rb_tree::create_node(node **pNode, node *parent, _color color, const int &low, const int &high)
{
    *pNode = new node;
    if(!*pNode)
...
bool rb_tree::insert(const int & low, const int &high)
{
    cout<<"insert:"<<low<<endl;
    node** walk = &root;
    node* parent = NULL;
    ...
    if(!(*walk))
    {
        create_node(walk,NULL,color_for_root,low, high);
        return true;
    }
    while((*walk)!=NIL)
    {
    ...
    create_node(walk,parent,color_for_new,low, high);
    maintain_size(*walk);
    rb_insert_fix(*walk);
    interval_tree_fix(*walk);
    return true;
}
void rb_tree::interval_tree_fix(node* pNode)
{
    node* walk = pNode->parent;
    while(walk!=NULL)
    {
       if(((walk->left == NIL)&&(walk->right!=NIL))||((walk->left!=NIL)&&(walk->right==NIL)))
        {
            node* tmp;
            if(walk->left==NIL)
                 tmp = walk->right;
            else tmp = walk->left;
            walk->max = (walk->high)>tmp->max?walk->high:tmp->max;
        }
        else
        {
            int big = 0;
            if(walk->high > walk->left->max)
                big = walk->high;
            else big = walk->left->max;

             walk->max = big > walk->right->max?big:walk->right->max;
        }
        walk = walk->parent;
    }
}
