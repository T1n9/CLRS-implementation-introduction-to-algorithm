...
public:
    hash_table();
    ~hash_table();
    void insert(const table_node& node);
     int able_to_push(const table_node&);
    void resize();
...
void hash_table::insert(const table_node &node)
{
    int result = 0;
...
int hash_table::able_to_push(const table_node & node)
{
    int iter = 0;
    int i = 0;
